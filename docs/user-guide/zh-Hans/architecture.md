...
12. [重连与恢复](#重连与恢复)
13. [设置与持久化](#设置与持久化)
14. [云同步、备份与便携包](#云同步备份与便携包)
15. [OxideSens AI 架构](#oxidesens-ai-架构)
16. [插件架构](#插件架构)
17. [CLI 伴侣工具边界](#cli-伴侣工具边界)
18. [安全设计](#安全设计)
19. [性能设计](#性能设计)
20. [详细模块结构](#详细模块结构)
21. [数据流详解](#数据流详解)
22. [状态机与生命周期](#状态机与生命周期)
23. [所有权与持久化矩阵](#所有权与持久化矩阵)
24. [事件与通知模型](#事件与通知模型)
25. [故障模型](#故障模型)
26. [Native Crate 对照](#native-crate-对照)
27. [Tauri 参考映射](#tauri-参考映射)
...
包含凭据的数据必须进入凭据感知存储，而不是普通 JSON 或文本字段。
...
云同步、备份和 `.oxide` 包作用于持久化状态，不作用于 在线终端字节。
...
OxideSens 是理解工作区上下文的助手。它使用已配置供应商 和本地应用上下文，不需要 OxideTerm 账号。
...
| `workspace/connection_monitor/*` | 跟踪连接池状态、节点健康、拓扑和生命周期动作 | 用户可以看到节点是已连接、重连中、失败还是空闲 |
...
| `workspace/settings/*` | 渲染终端、外观、AI、SFTP、IDE、便携运行时、更新和快捷键设置页 | 配置以应用为主入口，并通过共享设置模型持久化 |
...
| SSH、节点路由、重连、监控集成 | `oxideterm-ssh`, `oxideterm-topology`, `oxideterm-connection-monitor` |
...
| 设置 | `oxideterm-settings`, `oxideterm-settings-model`, `oxideterm-gpui-settings-view` |
