...
<p align="center">
  <strong>面向终端、文件、端口与远程上下文的 AI 原生 SSH 工作区。</strong>
  <br>
  <strong>零 Electron。零 OpenSSL。零遥测。零订阅。纯 Rust SSH。</strong>
  <br>
  <em>一个原生二进制，集成本地 Shell、SSH、SFTP、端口转发、远程编辑、插件与 OxideSens AI。</em>
</p>
...
> [!NOTE]
> **许可证变更：** 从 v1.0.0 起，OxideTerm 已将许可证从 **PolyForm Noncommercial 1.0.0** 变更为 **GPL-3.0（GNU 通用公共许可证 v3.0）**。这意味着 OxideTerm 现在是完全开源的——您可以在 GPL-3.0 许可证条款下自由使用、修改和分发。我们主动做这个转换，就是为了不玩带非商用陷阱或竞业禁止附加条款的“开源 cosplay”，而是把用户、fork、再分发者和商业使用者的 copyleft 自由说清楚。详见 [LICENSE](../../LICENSE) 文件。
