...
<p align="center">
  ...
  <br>
  <em>如果你想要一个没有 Electron、遥测或订阅的本地优先 SSH 工作区，请给 OxideTerm 点个 Star，让更多 SSH 用户发现它。</em>
</p>


<p align="center">
  <strong>OxideTerm 是本地优先的 SSH 工作区，不只是一个终端。</strong>
  <br>
  <em>打开一次远程节点（服务器连接），就围绕它使用 Shell、SFTP、端口转发、trzsz、轻量编辑和 BYOK AI。</em>
  <br>
  <em>打开一个 SSH 节点，就能在同一个本地工作区管理它的 Shell、文件、端口、编辑器和 AI 上下文。</em>
  <br>
  <strong>零 Electron。零 OpenSSL。零遥测。零订阅。BYOK 优先。纯 Rust SSH。</strong>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/version-1.4.2-blue" alt="版本">
...
*🤖 OxideSens AI —— 从一个助手控制实时终端与工作区工具。*
...
| 本地 Shell 也在同一套工作流里 | **混合引擎**：本地 PTY（zsh/bash/fish/pwsh/WSL2）与远程 SSH 同窗共存，本地和远程工作不再割裂 |
| SSH 工作流不需要云账号 | **本地优先核心**：SSH、SFTP、端口转发、本地 Shell 和配置管理都无需注册 |
| BYOK AI，而不是平台点数 | **OxideSens**：使用你自己的 OpenAI/Ollama/DeepSeek/OpenAI-compatible 端点，并支持 MCP 与 RAG |
| 不想要 Electron 运行时 | **Tauri 2.0**：原生 Rust 后端，25–40 MB 二进制文件 |
| 不想背 OpenSSL 包袱 | **russh 0.59**：基于 `ring` 编译的纯 Rust SSH——零 OpenSSL/libssh2 依赖 |
| 不想要遥测或应用订阅 | **零追踪，核心 SSH 工作流零应用订阅**：SSH/SFTP/端口转发/本地 Shell 不需要账号或订阅；数据默认留在你的设备上；云同步通过[官方插件](#官方插件)按需启用 |
| 连接稳定性 | **宽限期重连**：断开前探测旧连接 30 秒——网络抖动时 vim/htop/yazi 仍有机会存活 |
| 不想为了远程文件就打开 VS Code Remote | **内置 SFTP + 轻量 IDE**：在同一个 SSH 工作区内浏览、预览、传输和编辑远程文件 |
| 凭证安全 | **静态加密**：密码和 API 密钥保存在 OS 密钥链中，已保存连接的元数据在本地密封存储，`.oxide` 文件使用 ChaCha20-Poly1305 + Argon2id 加密 |

## 它是什么 / 不是什么

OxideTerm 是一个**本地优先的 SSH 工作区**：打开一次远程节点，就在同一处操作 Shell、文件、端口、终端内传输、轻量编辑和 AI 上下文。

OxideTerm **不是**云端 AI 平台、托管 Agent 服务、远程协议工具箱，也不以终端渲染性能跑分为主要卖点。许多现代终端正在围绕本地 Shell、AI 面板或云端 Agent 平台演进；OxideTerm 则专注于本地优先的 SSH 工作区。
