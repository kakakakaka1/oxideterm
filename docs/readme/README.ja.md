...
OxideTerm は SSH とリモート運用のためのオープンソースワークスペースです。ターミナル、ファイル、ポート転送、ホストツール、Raw ソケット、リモートデスクトップを一つの画面で扱えます。

**できること：**

- SSH、Telnet、シリアル、RDP/VNC、SFTP、ポート転送、Raw TCP/UDP ソケット、ローカルシェル、軽量編集を一つのワークスペースで管理
...
| カテゴリ | 機能 |
|---|---|
| **ターミナルと接続** | ローカルシェル、SSH、Telnet、シリアル、Raw TCP/UDP、分割ペイン、マルチホップ、安定した再接続 |
...
OxideTerm はターミナル、SSH、Telnet、RDP、VNC、Raw TCP/UDP、SFTP、ポート転送、IDE、AI、プラグイン、CLI を一つの Rust アーキテクチャにまとめています。技術的な詳細を以下に示します。
