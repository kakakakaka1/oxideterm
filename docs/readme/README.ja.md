...
<p align="center">
  ...
  <br>
  <em>Electron、テレメトリ、サブスクなしの local-first SSH ワークスペースがほしいなら、OxideTerm に Star を付けて、より多くの SSH ユーザーに見つけてもらえるよう応援してください。</em>
</p>
...
<p align="center">
  <img src="https://img.shields.io/badge/version-1.4.3-blue" alt="バージョン">
...
*🤖 OxideSens AI — ライブターミナルとワークスペースツールを 1 つのアシスタントから操作。*
...
| カテゴリ | 機能 |
|---|---|
| **ターミナル** | ローカル PTY（zsh/bash/fish/pwsh/WSL2）、SSH リモート、分割ペイン、ブロードキャスト入力、セッション録画・再生（asciicast v2）、WebGL レンダリング、30 以上のテーマ + カスタムエディター、コマンドパレット（`⌘K`）、Zen モード、**trzsz** 帯域内ファイル転送 |
