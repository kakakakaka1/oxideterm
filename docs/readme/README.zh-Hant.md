...
<p align="center">
  ...
  GPU 直接渲染。免費，無需註冊。
  <br>
  <strong>不捆綁 WebView。不蒐集遙測。無需訂閱。BYOK 優先。純 Rust SSH，不依賴 OpenSSL/libssh2。</strong>
</p>


<p align="center">
  <img src="https://img.shields.io/badge/version-2.0.0--gpui--preview.15-blue" alt="版本">
...
| 純 Rust SSH 與憑證安全 | SSH 堆疊使用 `russh` + `ring`，不依賴 OpenSSL/libssh2；已儲存憑證使用系統鑰匙圈，`.oxide` 使用 ChaCha20-Poly1305 + Argon2id |
...
原生版本把與 Tauri 版本同源的 `russh` 堆疊直接連結進桌面應用二進位：
...
OxideSens 仍然是 BYOK 優先，原生版本把上下文構建放在行程內完成：
...
日常開發優先檢查單一 crate；變更跨越 crate 邊界時，再檢查整個工作區。
...
已有 Tauri 功能遷移到原生版本時，應保持行為、標籤、互動狀態和工作流程一致，除非明確記錄替代設計。新 crate 必須承擔真實職責，不能只是重新匯出或堆放函式。
