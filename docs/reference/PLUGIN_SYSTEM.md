...
其中 `ctx.sync` 提供保存连接的加密 `.oxide` 导出/导入、记录级冲突预览，以及 `rename` / `skip` / `replace` / `merge` 四种冲突策略；预览结果现在除了汇总字段，还会返回每条记录的 `action` 与 `reasonCode`，以及 `hasAppSettings` / `pluginSettingsCount`，便于同步插件直接渲染“为什么会 rename / skip / replace / merge”，并提示这份快照是否还会恢复全局设置与插件偏好。`ctx.sync.exportOxide()` 默认仍只打包连接/凭据/forward；如果同步插件希望额外携带全局设置与声明式 plugin settings，需要显式传入 `includeAppSettings` / `includePluginSettings`。`ctx.secrets` 提供插件作用域的 OS keychain 安全存储，适合保存 WebDAV token、同步密码等敏感信息。当前 `merge` 是非破坏式合并：连接主字段采用导入值，`tags` 做并集，本地已保存但导入缺失的 password / passphrase 会保留；`.oxide` 里的 `forwards` 现在会作为 owner-bound saved forward 一起持久化导入导出，但不会在导入时直接变成活跃转发。加密快照本身还可以捆绑全局 OxideTerm settings 快照和声明式 plugin settings 偏好快照。同步插件还可以通过 `ctx.forward.listSavedForwards()` / `exportSavedForwardsSnapshot()` 独立同步保存转发，并通过 `ctx.settings.exportSyncableSettings()` / `applySyncableSettings()` 读写宿主白名单设置，拿到字段级 warnings 和归一化后的实际应用结果。
