...
use crate::{
    ...
    assets::{DynamicTexture, DynamicTextureParams},
    point,
    prelude::*,
...
fn dynamic_texture_byte_len(size: Size<DevicePixels>) -> Result<usize> {
    let width = usize::try_from(size.width.0).context("invalid dynamic texture width")?;
    let height = usize::try_from(size.height.0).context("invalid dynamic texture height")?;
    width
        .checked_mul(height)
        .and_then(|pixels| pixels.checked_mul(4))
        .context("dynamic texture byte size overflow")
}
...
fn validate_dynamic_texture_update(
    ...
    if bounds.origin.x.0 < 0
        || bounds.origin.y.0 < 0
        || bounds.right().0 > texture_size.width.0
        || bounds.bottom().0 > texture_size.height.0
    {
    ...
    if bytes.len() != expected_len {
        return Err(anyhow!(
            "dynamic texture update byte length mismatch: expected {}, got {}",
            expected_len,
            bytes.len()
        ));
    }
...
impl Window {
    ...
    /// Upload a BGRA region into a dynamic texture without replacing the texture identity.
    ///
    ...
    pub fn update_dynamic_texture(
        ...
        let params = DynamicTextureParams {
            texture_id: data.id,
        };
        let key = params.into();
        let size = data.size();
        self.sprite_atlas
            .get_or_insert_with(&key, &mut || {
                // The first update may arrive before the texture is painted, so create
                // a stable blank atlas entry and then upload the dirty region below.
    ...
    /// Paint a dynamic texture into the scene for the next frame at the current z-index.
    ///
    /// This method reuses one atlas entry across frames and is intended for mutable
    /// surfaces such as remote desktop framebuffers.
    ...
    /// Removes a dynamic texture from the sprite atlas.
