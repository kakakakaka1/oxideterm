...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum ButtonRadius {
    None,
    Sm,
    Md,
}

pub(crate) fn button(tokens: &ThemeTokens, label: String, tone: ButtonTone) -> Div {
    button_with(
        tokens,
        label,
        ButtonOptions {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct ButtonOptions {
...
pub(crate) fn button_with(tokens: &ThemeTokens, label: String, options: ButtonOptions) -> Div {
    let theme = tokens.ui;
